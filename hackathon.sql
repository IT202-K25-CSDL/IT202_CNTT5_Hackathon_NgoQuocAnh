...
INSERT INTO tickets (show_id, customer_id, seat_number, status) 
VALUES ('S01', 'C01', 'A01', 'Booked'), 
('S02', 'C02', 'B05', 'Booked'), 
...
WHERE customer_id IN (SELECT customer_id FROM tickets WHERE status = 'Cancelled');
...
-- 7
SELECT full_name, email FROM customers WHERE full_name LIKE '%Mai';

-- 8
SELECT t.show_id, s.room_name, s.start_time
FROM tickets t
JOIN showtimes s ON t.show_id = s.show_id
ORDER BY s.start_time DESC;

-- 9
SELECT * FROM showtimes ORDER BY ticket_price DESC LIMIT 3;

-- 10
SELECT title, duration FROM movies LIMIT 3 OFFSET 1;
